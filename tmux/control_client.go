...
func (cc *ControlClient) attach(w io.Writer, closeFn func() error) {
	...
	// advertise the previous connection as current for the length of this
	// write. Write order is unaffected, since the stdinMu section below spans
	// both the cc.stdin assignment and the write.
	cc.connMu.Lock()
	cc.closeCur = closeFn
	cc.connOK = true
